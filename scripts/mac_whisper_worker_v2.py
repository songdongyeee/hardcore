...
# -*- coding: utf-8 -*-
"""
Mac Whisper 转写工作进程 (Requests 版本 - 更稳定的网络连接)

在你的 Mac 上运行，自动处理服务器上标记为 whisper 的转写任务

用法:
  python3 mac_whisper_worker_v2.py
"""
...
class WhisperWorker:
    ...
    def fetch_pending_tasks(self):
        """获取待处理的转写任务"""
        try:
            url = f"{self.pb_url}/api/collections/transcripts/records"
            params = {
                'filter': "asr_engine='whisper' && status='pending'"
            }
    ...
    def update_record(self, record_id, transcription_data, status='done'):
        ...
        try:
            ...
            data = {
                "text": json.dumps(transcription_data) if transcription_data else "",
                "status": status
            }
    ...
    def process_task(self, record):
        ...
        try:
            url = f"{self.pb_url}/api/collections/transcripts/records/{record_id}"
            self.session.patch(url, json={"status": "processing"}, timeout=30)
        except:
            pass
    ...
    def run(self):
        """主循环"""
        self.log("🚀 Mac Whisper Worker v2 启动 (Requests版)")
        ...
        while True:
            try:
                ...
                retry_count += 1
                self.log(f"⚠️ 循环错误 ({retry_count}/{max_retries}): {e}")
                
                if retry_count >= max_retries:
                    self.log("❌ 连续错误过多，尝试重新登录...")
                    self.login_admin()
                    retry_count = 0
