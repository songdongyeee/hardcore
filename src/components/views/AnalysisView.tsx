...
export function AnalysisView({ onBack, onNextPhase, audioRef: _audioRef, currentTime, isPlaying: _isPlaying, seek: _seek, transcript }: AnalysisViewProps) {
  ...
  return (
    <div className="absolute inset-0 bg-black z-40 flex flex-col h-full w-full">
      ...
      <div className="flex-1 overflow-y-auto p-8 flex items-start justify-center relative no-scrollbar" ref={scrollBoxRef}>
        <div className="space-y-10 text-2xl md:text-3xl font-sans font-medium leading-relaxed tracking-wide text-left max-w-xl text-zinc-300 pb-20">
          {transcript.map((seg, idx) => {
            return (
              <div key={idx} className="mb-10">
                <p className="leading-loose transition-all duration-300">
                  {seg.words?.map((word, wIdx) => {
                    // Match active word OR the most recent word (within 2 seconds after it ended)
                    // This ensures scroll positioning works even when audio is paused between words
                    ...
                    const shouldHighlight = isWordActive || (isRecentWord && !hasScrolledRef.current);
                    const refProps = shouldHighlight && !hasScrolledRef.current ? { ref: activeWordRef } : {};

                    const isSelected = selectedWordEntry?.word.toLowerCase() === word.text.toLowerCase().replace(/[^a-z]/g, "");

                    let isHidden = false;
                    const isRealWord = /\p{L}|\p{N}/u.test(word.text); // 🔥 Fixed: Use Unicode regex
                    if (isRealWord) {
                    ...
                    return (
                      <span key={wIdx} className="inline-block mr-1.5 relative group">
                        <span
                          ...
                          className={cn(
                            "transition-all duration-200 cursor-pointer rounded-md border-b border-zinc-900/50 hover:border-zinc-700 hover:bg-zinc-800",
                            ...
                            // UPDATED MASK STYLES (Blocks)
                            isHidden && !isWordActive && "bg-zinc-800 text-transparent select-none rounded-md", // Redacted Block
                            isHidden && isWordActive && "bg-indigo-500 text-transparent rounded-md animate-pulse" // Active Redacted
                          )}
                        >
                          {word.text}
                        </span>
                ...
                {/* Translation with smooth reveal animation */}
                {seg.translation && (
                  <div
                    className={cn(
                  ...
                  >
                    <div className="overflow-hidden">
                      <div className="text-sm text-zinc-500 font-medium leading-relaxed pl-1 border-l-2 border-indigo-500/30">
                        {seg.translation}
                      </div>
                    </div>
                  </div>
